...
class SoundClient:
    def __init__(self, root):
        ...
        tk.Button(root, text="Play", command=lambda: self.send_action("start")).pack(pady=5)
        tk.Button(root, text="Stop", command=lambda: self.send_action("stop")).pack(pady=5)
        tk.Button(root, text="Subir frecuencia", command=lambda: self.send_action("freq_up")).pack(pady=5)
        tk.Button(root, text="Bajar frecuencia", command=lambda: self.send_action("freq_down")).pack(pady=5)
